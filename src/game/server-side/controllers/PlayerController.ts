            ...
            if (room) {
                ...
                if (key !== KeyTypes.SPACE) {
                    if (isKeyDown) {
                        switch (key) {
                            case KeyTypes.UP:
                                player?.moveOn(Movements.UP)
                                break
                            case KeyTypes.RIGHT:
                                player?.moveOn(Movements.RIGHT)
                                break
                            case KeyTypes.DOWN:
                                player?.moveOn(Movements.DOWN)
                                break
                            case KeyTypes.LEFT:
                                player?.moveOn(Movements.LEFT)
                                break
                            default:
                                player?.moveOn(Movements.NONE)
                                break
                        }
                    } else {
                        player?.moveOff()
